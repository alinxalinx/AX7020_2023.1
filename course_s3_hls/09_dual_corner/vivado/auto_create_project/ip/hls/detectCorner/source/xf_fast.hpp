...
void ProcessFast(xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& _src_mat,
				 xf::cv::Mat<XF_8UC3, ROWS, COLS, NPC>& _src,
                 xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& _out_mat,
				 xf::cv::Mat<XF_8UC3,ROWS,COLS,XF_NPPC1>&_dst_mat,
				 XF_TNAME(XF_8UC3,XF_NPPC1)pixel_src1[COLS],
...
// clang-format off
	...
	XF_TNAME(XF_8UC3,XF_NPPC1)pixel_src2[COLS];
...
// clang-format off
        ...
        if (row < img_height && col < (img_width >> XF_BITSHIFT(NPC)))
		{
			...
			pixel_src2[col]=_src.read(read_index);
			read_index++;
		}
        if (NPC == XF_NPPC8) {
...
// clang-format off
            ...
            if (col >= 1) {
				...
				if(en==0)
				{
				_dst_mat.write(write_index,pixel_src1[col]);
				}
				else
				{
				_dst_mat.write(write_index,pixel_src2[col]);
				}
				write_index++;
			}
...
// clang-format off
			...
			pixel_src1[col]=_src.read(read_index);
			read_index++;
        }
    }
...
// clang-format off
        ...
        ProcessFast<SRC_T, ROWS, COLS, DEPTH, NPC, WORDWIDTH, TC, WIN_SZ, WIN_SZ_SQ>(
            _src_mat, _src,_out_mat, _dst_mat, pixel_src1,buf, src_buf, OutputValues, P0, en,img_width, img_height, shift_x, row_ind, row, win_size,
...
void xFFastCornerDetection(xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& _src_mat,
        ...
        xFfast7x7<SRC_T, ROWS, COLS, DEPTH, NPC, WORDWIDTH_SRC, (COLS >> XF_BITSHIFT(NPC)) + (7 >> 1), 7, 7 * 7>(
            _src_mat, _src,_dst_mat, _dst,7, _image_height, _image_width, _threshold);
}
...
// clang-format off
    ...
    xFFastCornerDetection<SRC_T, ROWS, COLS, XF_DEPTH(SRC_T, NPC), NPC, XF_WORDWIDTH(SRC_T, NPC), XF_32UW, NMS>(
        _src_mat,_src, _dst_mat, _dst,_src_mat.rows, _src_mat.cols, _threshold);
}
} // namespace cv
} // namespace xf
#endif //_XF_FAST_HPP_
