...
module ds1302_io(
	...
	output          ds1302_sclk,
	inout           ds1302_io,
	input           cmd_read,		//read command
	input           cmd_write,		//write command
	output          cmd_read_ack,	//read command respond
	output          cmd_write_ack,	//write command respond
	input[7:0]      read_addr,		//read address
...
reg	[7:0] 	send_data;	//data send to spi master
wire[7:0] 	data_rec;	//data received from spi master
//bidirection io
reg ds1302_io_dir;
	...
	case(state)
		S_IDLE:
			if(cmd_read || cmd_write)
				...
				next_state <= S_IDLE;
		S_CE_HIGH:
			if(delay_cnt == 20'd255)		//set ce to high delay some periods
				next_state <= cmd_read ? S_READ : S_WRITE;	//check command
			else
				next_state <= S_CE_HIGH;
		S_READ:
			next_state <= S_READ_ADDR;
		S_READ_ADDR:						//read address
			if(wr_ack)						//spi master ack
				next_state <= S_READ_DATA;
			else
				next_state <= S_READ_ADDR;
		S_READ_DATA:						//read data
			if(wr_ack)
				next_state <= S_ACK;
			else
				next_state <= S_READ_DATA;
...
//ds1302 data io direction control
always@(posedge clk or posedge rst)
begin
	if(rst)
		ds1302_io_dir <= 1'b0;
...
//ce control, when in S_CE_HIGH,set it to high, or low
always@(posedge clk or posedge rst)
begin
	if(rst)
		CS_reg <= 1'b0;
...
//read data from spi master and adjust data sequence, for ds1302, LSB first
always@(posedge clk or posedge rst)
begin
	if(rst)
		read_data <= 8'h00;
...
begin
	if(rst)
		send_data <= 8'h00;
	else if(state == S_READ_ADDR)		//LSB first, for read, LSB is high
		send_data <= {1'b1,read_addr[1],read_addr[2],read_addr[3],read_addr[4],read_addr[5],read_addr[6],1'b1};
	else if(state == S_WRITE_ADDR)		//LSB first, for write, LSB is low
