...
int XAxiDma_Adc_Wave(u32 width, u8 *frame, u32 stride, XScuGic *InstancePtr)
{
	...
	/* Create BD chain */
	CreateBdChain(BdChainBuffer, BD_COUNT, ADC_BYTE*ADC_CH_COUNT*ADC_CAPTURELEN, (u8 *)DmaRxBuffer, RXPATH) ;
	/* Grid Overlay */
	...
	while(1)
	{
		...
		if (s2mm_flag)
		{
			...
			for(i = 0; i < 8 ; i++)
			{
				draw_wave(wave_width, WAVE_HEIGHT, (void *)DmaRxBufferTmp[i], WaveBuffer, SHORT, ADC_BITS, i, ADC_COE+10*i) ;
			}
			...
			/* start DMA translation from AD7606 to DDR3 */
