...
//================================================================================
//   Description:  buffer from axis interface to DAC
//
//================================================================================
//  Revision History:
//  Date          By            Revision    Change Description
//--------------------------------------------------------------------------------
//  2019/5/15     myj          1.0         Original
//********************************************************************************/
`timescale 1 ns/1 ns
module ad9708_send(
    input                       dac_clk,
...
wire [7:0]   dac_buf_data ;  //fifo read data
reg          fifo_wr_en   ;  //fifo write enable
reg [7:0]    fifo_wr_data ;  //fifo write data
...
begin
	...
	begin
		...
		case(state)
			S_IDLE:
			begin
			  if (send_start_d2 && rd_data_count > 10'd256)    /* When there are some data in fifo, start to read fifo data for DAC  */
			  begin
				state  <= S_SEND ;
			  end		    
			end
...
/* When S_AXIS_tready and S_AXIS_tvalid both valid, write data from S AXI4-Stream to fifo*/
