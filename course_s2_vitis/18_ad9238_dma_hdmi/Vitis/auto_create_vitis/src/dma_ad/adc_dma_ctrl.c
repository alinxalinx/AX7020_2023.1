...
int XAxiDma_Adc_Wave(u32 width, u8 *frame, u32 stride, XScuGic *InstancePtr)
{
	...
	while(1)
	{
		...
		if (ch0_s2mm_flag & ch1_s2mm_flag)
		{
			...
			Status = XAxiDma_SimpleTransfer(&AxiDmaCh0,(UINTPTR) CH0DmaRxBuffer,
					ADC_BYTE*ADC_CAPTURELEN, XAXIDMA_DEVICE_TO_DMA);
			if (Status != XST_SUCCESS) {
				return XST_FAILURE;
			}
			/* start DMA translation from AD9238 channel 1 to DDR3 */
			Status = XAxiDma_SimpleTransfer(&AxiDmaCh1,(UINTPTR) CH1DmaRxBuffer,
					ADC_BYTE*ADC_CAPTURELEN, XAXIDMA_DEVICE_TO_DMA);
